...
Простий Android-додаток для планшета **Samsung Galaxy Tab A9 (Android 16, вертикальна орієнтація)**.
...
**Довідник вшитий у додаток** (`android/app/src/main/assets/catalog/products.xlsx`, 688 позицій):
при першому запуску він завантажується автоматично, імпорт не потрібен. Якщо в новій версії APK
файл оновлено, довідник на планшеті оновиться сам, але тільки якщо оператор не імпортував
власний файл. Повернути вшитий довідник: **Налаштування → Відновити вбудований довідник**.
Щоб оновити вшитий довідник, замініть файл `products.xlsx` і зробіть push: GitHub Actions збере новий APK.

Імпорт іншого файлу: **Налаштування → Імпорт CSV / XLSX**. Додаток читає файл, знаходить рядок заголовків
...
4. Довідник уже вшитий у додаток. Імпорт CSV / XLSX потрібен лише для іншого списку.
